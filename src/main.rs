...
///
/// if `c` is not a member, return `Some(i)`, where `i` is the member of
...
fn test_parse_pair()
{
    ...
    assert_eq!(parse_pair::<i32>("10,10x", ','), None);
    assert_eq!(parse_pair::<f64>("0.5x", 'x'), None);
    assert_eq!(parse_pair::<f64>("0.5x0.3", 'x'), Some((0.5, 0.3)));
}
...
/// which holds on grayscale pixel per byte. The `upper_left` and `lower_right`
...
/// Write the buffer `pixels`, whose dimensions are given by the `bounds`,
